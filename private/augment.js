 ...
 * @param {String[]} path            A list of keys of increasing depth that end in the object that
 *                                   will receive the id/value pair.
 * @param {String}   id              The key in the final sub-object in the path that will receive
 *                                   value.
...
module.exports = function (panopticon, DataConstructor, path, id, value) {
	...
	if (data[id]) {
		...
		data[id] = new DataConstructor(
			value,
			panopticon.endTime,
			panopticon.persist ? panopticon : null,
			panopticon.scaleFactor,
			panopticon.interval
		);
	}
};
